...
{
	cout << "Счёт " << win << " : " << lose;
	if (win > lose)cout << " в пользу ИГРОКА\n";
	else if (win < lose)cout << " в пользу КОМПЬЮТЕРА\n";
	else cout << " равный.\n";
}
...
int main()
{
	...
	while (true) {
		...
		ai = rand() % 2;
		Sleep(1000);
		if (ai == true) cout << "Первым ходит КОМПЬЮТЕР";
		else cout << "Первым ходит ИГРОК";
		cout << endl;
		Sleep(1000);
		while (true)
		{
